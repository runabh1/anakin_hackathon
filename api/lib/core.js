...
export function buildPrompt(profile) {
  return [
    `Research Indian college admissions and scholarships for a first-generation student:`,
    `Location: ${profile.location||"India"}, Stream: ${profile.stream||"PCM"},`,
    `Category: ${profile.category||""}, Income: ${profile.income||""},`,
    `Exams: ${profile.exams||"JEE"}, Career: ${profile.career||"engineering"}.`,
    `1. Top 5 affordable Indian colleges (NIT/IIT/state universities) with fees and cutoffs.`,
    `2. Top 5 active Indian scholarships (NSP/AICTE/state/private) with deadlines, amounts, eligibility.`,
...
function personalizeScholarships(profile, list) {
  return list.map(s => ({
    ...
    deadline:    s.deadline || s.last_date || s.closeDate || "Check current listing",
...
export const FALLBACK_SCHOLARSHIPS = [
  { name:"Central Sector Scholarship (NSP)", amount:"Up to ₹12,000/yr UG", deadline:"Check current NSP cycle",     eligibility:"High Class 12 merit + income criteria",              why:"Built for strong Class 12 students from lower-income homes.",          source:"NSP" },
  { name:"Post Matric Scholarship",          amount:"Tuition + maintenance", deadline:"State NSP deadline",         eligibility:"Category + income + domicile rules",                 why:"Reduces fee burden directly if category and income match.",            source:"NSP / state portal" },
  { name:"AICTE Pragati Scholarship",        amount:"Up to ₹50,000/yr",     deadline:"Check AICTE/NSP cycle",      eligibility:"Girl students in AICTE-approved tech programs",      why:"Strong fit for a first-generation girl entering technical education.", source:"AICTE / NSP" },
  { name:"Vidyasaarathi Scholarships",       amount:"Varies by sponsor",    deadline:"Varies by listing",          eligibility:"Marks, income, state, course rules vary",            why:"Private scholarship backup beyond government aid.",                    source:"Vidyasaarathi" },
  { name:"Buddy4Study Scholarships",         amount:"Varies by program",    deadline:"Varies by listing",          eligibility:"Marks, income, location, course rules vary",         why:"Find live private scholarships matched to course and income.",         source:"Buddy4Study" },
];
